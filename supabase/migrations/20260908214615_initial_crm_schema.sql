...
do $$ begin
  create type public.crm_stage as enum ('new_lead','waiting_response','waiting_confirmation','scheduled','attended','follow_up','recurring','cancelled');
exception when duplicate_object then null; end $$;
...
create index if not exists idx_client_consents_client_category on public.client_consents(client_id, category, recorded_at desc);
...
create index if not exists idx_whatsapp_messages_client_created on public.whatsapp_messages(client_id, created_at desc);
...
begin
  foreach t in array array[
    'services','service_packages','clients','client_consents','availability_rules','blocked_periods','appointments','appointment_events','whatsapp_contacts','whatsapp_conversations','whatsapp_messages','message_templates','quick_replies','campaigns','campaign_recipients','notification_jobs','audit_logs'
  ] loop
    execute format('drop policy if exists "admin_all" on public.%I', t);
    execute format('create policy "admin_all" on public.%I for all to authenticated using (exists (select 1 from public.profiles p where p.id = (select auth.uid()) and p.active = true and p.role = ''admin'')) with check (exists (select 1 from public.profiles p where p.id = (select auth.uid()) and p.active = true and p.role = ''admin''))', t);
  end loop;
end $$;
...
insert into public.services (slug,name,description,duration_minutes,active,sort_order) values
...
insert into public.quick_replies (shortcut,title,body) values
